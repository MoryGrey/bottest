...
function updateLoyaltyCounter() {
	...
	if (freeRidesElement && ridesToFreeElement && progressFill) {
		// Update counters
		const currentFreeRides = parseInt(freeRidesElement.textContent) + 1;
		const currentRidesToFree = parseInt(ridesToFreeElement.textContent) - 1;
		
		freeRidesElement.textContent = currentFreeRides;
		ridesToFreeElement.textContent = currentRidesToFree;
		...
		setTimeout(() => {
			alert('Данные обновлены! Счётчик до бесплатной поездки уменьшен на 1.');
		}, 1000);
	}
}
