(function(){
	...
	const userPrefill = {
		...
		usedFreeRides: parseInt(qs.get('used_free_rides') || '0'),
	...
	const earnedFreeRides = Math.floor(userPrefill.totalCompleted / 10);
	const availableFreeRides = earnedFreeRides - (userPrefill.usedFreeRides || 0);
	const ridesToFree = userPrefill.ridesToFree;
	const progressPercent = ((userPrefill.sinceFree % 10) / 10) * 100;
	
	document.getElementById('free-rides').textContent = availableFreeRides;
	document.getElementById('rides-to-free').textContent = ridesToFree;
	document.getElementById('progress-fill').style.width = `${progressPercent}%`;
	
	// Показываем опцию бесплатной поездки если есть доступные
	const freeRideOption = document.getElementById('free-ride-option');
	if (availableFreeRides > 0) {
		freeRideOption.style.display = 'flex';
	} else {
		freeRideOption.style.display = 'none';
	}
	...
	const paymentNames = { cash: 'Наличными', card: 'Перевод', free: 'Бесплатная поездка' };
	...
	function updateSummary() {
		...
		let totalPrice = Math.max(0, basePrice + stopsPrice - promoDiscount);
		
		const tariffName = tariffNames[selectedTariff];
		const paymentName = paymentNames[selectedPayment];
		
		// Если выбрана бесплатная поездка, цена = 0
		...
		document.getElementById('total-price').textContent = selectedPayment === 'free' ? 'БЕСПЛАТНО' : `${totalPrice} ₽`;
	...
	async function refreshProfile() {
		...
		try {
			...
			if (data.ok) {
				...
				const usedFreeRides = data.trips.filter(t => t.status === "Выполнено" && t.payment === "free").length;
				const earnedFreeRides = Math.floor(completedTrips / 10);
				const availableFreeRides = earnedFreeRides - usedFreeRides;
				const ridesToFree = 10 - (completedTrips % 10);
				const progressPercent = ((completedTrips % 10) / 10) * 100;
				
				console.log('Updating counters:', { completedTrips, earnedFreeRides, usedFreeRides, availableFreeRides, ridesToFree, progressPercent });
				...
				// Обновляем опцию бесплатной поездки
				const freeRideOption = document.getElementById('free-ride-option');
				if (availableFreeRides > 0) {
					freeRideOption.style.display = 'flex';
				} else {
					freeRideOption.style.display = 'none';
					// Если бесплатных поездок нет, сбрасываем выбор на наличные
					if (selectedPayment === 'free') {
						selectedPayment = 'cash';
						document.getElementById('selected-payment-text').textContent = 'Наличными';
						updateSummary();
					}
				}
				...
				userPrefill.usedFreeRides = usedFreeRides;
			}
		} catch (e) {
